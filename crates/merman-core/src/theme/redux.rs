...
pub(crate) fn apply_redux_theme_defaults(tv: &mut ThemeVariables) {
    ...
    // Color scales — redux is monochromatic: ALL from mainBkg (white)
    let main_bkg = tv
        .main_bkg
        .as_deref()
        ...
        .unwrap_or("#ffffff")
        .to_string();
    let main_bkg_hsl = Rgb::try_from(&main_bkg).map(Hsl::from).unwrap_or(Hsl {
        h_deg: 0.0,
        s_pct: 0.0,
        l_pct: 100.0,
    });
    ...
    let c_scale_amount = main_bkg_hsl.adjust_hsl(0.0, 0.0, -amount).to_string();
    tv.set_c_scale0_if_none(c_scale_amount.clone());
    tv.set_c_scale1_if_none(c_scale_amount.clone());
    tv.set_c_scale2_if_none(c_scale_amount.clone());
    tv.set_c_scale3_if_none(c_scale_amount.clone());
    tv.set_c_scale4_if_none(c_scale_amount.clone());
    tv.set_c_scale5_if_none(c_scale_amount.clone());
    tv.set_c_scale6_if_none(c_scale_amount.clone());
    tv.set_c_scale7_if_none(c_scale_amount.clone());
    tv.set_c_scale8_if_none(c_scale_amount.clone());
    tv.set_c_scale9_if_none(c_scale_amount.clone());
    tv.set_c_scale10_if_none(c_scale_amount.clone());
    tv.set_c_scale11_if_none(c_scale_amount.clone());

    // All cScale values use mainBkg darkened, so they're the same
