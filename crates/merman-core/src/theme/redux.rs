...
pub(crate) fn apply_redux_theme_defaults(tv: &mut ThemeVariables) {
    ...
    tv.set_requirement_edge_label_background_if_none("#FFFFFF");
    ...
    let secondary_color = secondary_hsl.to_string();
    ...
    let tertiary_color = tertiary_hsl.to_string();
    ...
    // noteBkgColor = noteBkgColor || '#FEF9C3'
    tv.set_note_bkg_color_if_none("#FEF9C3");

    // noteTextColor = noteTextColor || '#28253D'
    tv.set_note_text_color_if_none("#28253D");
    ...
    // --- Local Gantt colors (used by Gantt, fillTypes, pie, quadrant, requirement, git) ---
    let gantt_primary_hsl = Rgb::try_from("#ECECFE").map(Hsl::from).unwrap_or(Hsl {
        h_deg: 240.0,
        s_pct: 90.0,
        l_pct: 96.0,
    });
    let gantt_primary = gantt_primary_hsl.to_string();

    let gantt_secondary_hsl = Rgb::try_from("#E9E9F1").map(Hsl::from).unwrap_or(Hsl {
        h_deg: 240.0,
        s_pct: 22.0,
        l_pct: 93.0,
    });
    let gantt_secondary = gantt_secondary_hsl.to_string();

    let gantt_tertiary_hsl = gantt_primary_hsl.adjust_hsl(180.0, 0.0, 5.0);
    let gantt_tertiary = gantt_tertiary_hsl.to_string();
    ...
    tv.set_actor_bkg_if_none(&main_bkg_str);
    tv.set_actor_text_color_if_none(&primary_text_color);
    tv.set_actor_line_color_if_none(&primary_border_color);

    // labelBoxBkgColor = actorBkg
    tv.set_label_box_bkg_color_if_none(&main_bkg_str);

    // signalColor = textColor (which = primaryTextColor)
    ...
    tv.set_activation_bkg_color_if_none(&secondary_color);
    ...
    tv.set_person_bkg_if_none(&main_bkg_str);

    // --- Gantt chart variables (using local Gantt colors) ---
    // sectionBkgColor = sectionBkgColor || tertiaryColor (local Gantt)
    // Note: JS has double assignment: first tertiaryColor, then secondaryColor (but || keeps first)
    tv.set_section_bkg_color_if_none(&gantt_tertiary);
    tv.set_alt_section_bkg_color_if_none("white");
    // sectionBkgColor = sectionBkgColor || secondaryColor -- no-op since already set by line above
    tv.set_section_bkg_color2_if_none(&gantt_primary);
    tv.set_exclude_bkg_color_if_none("#eeeeee");
    tv.set_task_border_color_if_none(&primary_border_color);
    tv.set_task_bkg_color_if_none(&gantt_primary);
    tv.set_active_task_border_color_if_none(&gantt_primary);
    // activeTaskBkgColor = lighten(primaryColor, 23) -- where primaryColor = local Gantt primary
    ...
    // stateBkg is set below; at this point it may not be set yet. If not set, fallback to primaryTextColor.
    tv.set_state_bkg_if_none(&main_bkg_str);
    let state_bkg_fallback = tv
        .state_bkg
        .as_deref()
        .filter(|s| !s.trim().is_empty())
        .unwrap_or(&primary_text_color)
        .to_string();
    tv.set_state_label_color_if_none(state_bkg_fallback);

    // labelBackgroundColor = stateBkg
    let state_bkg_val = tv
        .state_bkg
        .as_deref()
        .filter(|s| !s.trim().is_empty())
        .unwrap_or(&main_bkg_str)
        .to_string();
    tv.set_label_background_color_if_none(&state_bkg_val);
    ...
    if !is_truthy(&tv.composite_background) {
        let fallback = tv
            ...
            .as_deref()
            .filter(|s| !s.trim().is_empty())
            .unwrap_or(&tertiary_color);
        tv.composite_background = Some(fallback.to_string());
    }

    // altBackground = '#f0f0f0' (the || pattern; constructor already set '#F9F9FB', so this is no-op)
    tv.set_alt_background_if_none("#f0f0f0");

    // compositeTitleBackground = compositeTitleBackground || mainBkg
    tv.set_composite_title_background_if_none(&main_bkg_str);

    // compositeBorder = compositeBorder || nodeBorder
    let node_border_val = tv
        .node_border
        .as_deref()
        .filter(|s| !s.trim().is_empty())
        .unwrap_or(&primary_border_color)
        .to_string();
    tv.set_composite_border_if_none(&node_border_val);

    // innerEndBackground = nodeBorder
    tv.set_inner_end_background_if_none(&node_border_val);
    ...
    // --- Color Scales (cScale0-11) ---
    let amount = if dark_mode { 75.0 } else { 25.0 };
    // All cScales use mainBkg darkened by amount
    tv.set_c_scale0_if_none(main_bkg_hsl.adjust_hsl(0.0, 0.0, -amount).to_string());
    tv.set_c_scale1_if_none(main_bkg_hsl.adjust_hsl(0.0, 0.0, -amount).to_string());
    tv.set_c_scale2_if_none(main_bkg_hsl.adjust_hsl(0.0, 0.0, -amount).to_string());
    tv.set_c_scale3_if_none(main_bkg_hsl.adjust_hsl(0.0, 0.0, -amount).to_string());
    tv.set_c_scale4_if_none(main_bkg_hsl.adjust_hsl(0.0, 0.0, -amount).to_string());
    tv.set_c_scale5_if_none(main_bkg_hsl.adjust_hsl(0.0, 0.0, -amount).to_string());
    tv.set_c_scale6_if_none(main_bkg_hsl.adjust_hsl(0.0, 0.0, -amount).to_string());
    tv.set_c_scale7_if_none(main_bkg_hsl.adjust_hsl(0.0, 0.0, -amount).to_string());
    tv.set_c_scale8_if_none(main_bkg_hsl.adjust_hsl(0.0, 0.0, -amount).to_string());
    tv.set_c_scale9_if_none(main_bkg_hsl.adjust_hsl(0.0, 0.0, -amount).to_string());
    tv.set_c_scale10_if_none(main_bkg_hsl.adjust_hsl(0.0, 0.0, -amount).to_string());
    tv.set_c_scale11_if_none(main_bkg_hsl.adjust_hsl(0.0, 0.0, -amount).to_string());

    let c_scales: [Hsl; 12] = std::array::from_fn(|_| main_bkg_hsl.adjust_hsl(0.0, 0.0, -amount));
    ...
    // --- user-journey fill types (using local Gantt colors) ---
    tv.set_fill_type0_if_none(&gantt_primary);
    tv.set_fill_type1_if_none(&gantt_secondary);
    ...
    // --- Pie (using local Gantt colors) ---
    tv.set_pie1_if_none(&gantt_primary);
    tv.set_pie2_if_none(&gantt_secondary);
    tv.set_pie3_if_none(&gantt_tertiary);
    ...
    // pie title/section/legend settings
    ...
    // --- Quadrant (using local Gantt primary) ---
    tv.set_quadrant1_fill_if_none(&gantt_primary);
    // quadrant2Fill = adjust(primaryColor, {r: 5, g: 5, b: 5}) -- approximated via HSL
    tv.set_quadrant2_fill_if_none(gantt_primary_hsl.adjust_hsl(0.0, 0.0, 2.0).to_string());
    tv.set_quadrant3_fill_if_none(gantt_primary_hsl.adjust_hsl(0.0, 0.0, 4.0).to_string());
    tv.set_quadrant4_fill_if_none(gantt_primary_hsl.adjust_hsl(0.0, 0.0, 6.0).to_string());
    ...
    // quadrant1Fill = gantt_primary (#ECECFE) is a light color, so darken
    tv.set_quadrant_point_fill_if_none(gantt_primary_hsl.adjust_hsl(0.0, 0.0, -10.0).to_string());
    ...
    // --- Requirement diagram (using local Gantt primary) ---
    tv.set_requirement_background_if_none(&gantt_primary);
    ...
    // --- Git (using local Gantt colors) ---
    // git0-2 from local Gantt colors; git3-7 via adjust
    let git_adj = if dark_mode { 25.0 } else { -25.0 };

    let git_base = [
        gantt_primary_hsl,
        gantt_secondary_hsl,
        gantt_tertiary_hsl,
        gantt_primary_hsl.adjust_hsl(-30.0, 0.0, 0.0),
        gantt_primary_hsl.adjust_hsl(-60.0, 0.0, 0.0),
        gantt_primary_hsl.adjust_hsl(-90.0, 0.0, 0.0),
        gantt_primary_hsl.adjust_hsl(60.0, 0.0, 0.0),
        gantt_primary_hsl.adjust_hsl(120.0, 0.0, 0.0),
    ];

    let git: [Hsl; 8] = std::array::from_fn(|i| git_base[i].adjust_hsl(0.0, 0.0, git_adj));
    ...
    tv.set_commit_label_background_if_none(&secondary_color);
