...
pub(crate) fn apply_redux_dark_theme_defaults(tv: &mut ThemeVariables) {
    ...
    // secondaryColor = adjust(primaryColor, {h: -120}) [updateColors]
    ...
    // tertiaryColor = adjust(primaryColor, {h: 180, l: 5}) [updateColors]
    ...
    // primaryBorderColor = mkBorder(primaryColor, darkMode) [updateColors]
    ...
    // noteBorderColor = mkBorder(noteBkgColor, darkMode) — but constructor already set to "#FACC15",
    // and the || in JS keeps the constructor value, so this is already handled above.
    ...
    // --- Git ---
    let git_adj: f64 = 25.0; // lighten for dark mode
    let git_base: [Hsl; 8] = [
        primary_hsl,
        secondary_hsl,
        tertiary_hsl,
    ...
    let git = git_base.map(|g| g.adjust_hsl(0.0, 0.0, git_adj));
...
mod tests {
    ...
    static REDUX_DARK_THEME_JSON: &str = include_str!("../../assets/theme/redux-dark.json");

    #[test]
    fn compare_with_mermaid_theme_json() {
        let mut base = ThemeVariables::default();
        super::apply_redux_dark_theme_defaults(&mut base);
        let json = serde_json::to_string_pretty(&base).unwrap();
        let from_mermaid = serde_json::from_str::<ThemeVariables>(REDUX_DARK_THEME_JSON).unwrap();
