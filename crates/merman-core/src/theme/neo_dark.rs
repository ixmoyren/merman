...
mod tests {
    ...
    static NEO_DARK_THEME_JSON: &str = include_str!("../../assets/theme/neo-dark.json");

    #[test]
    fn compare_with_mermaid_theme_json() {
        let mut base = ThemeVariables::default();
        super::apply_neo_dark_theme_defaults(&mut base);
        let json = serde_json::to_string_pretty(&base).unwrap();
        let from_mermaid = serde_json::from_str::<ThemeVariables>(NEO_DARK_THEME_JSON).unwrap();
